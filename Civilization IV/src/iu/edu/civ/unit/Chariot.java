...
public class Chariot extends MilitaryUnit {
	static{
		baseStrength = 4;
		cost = 37;
		upgradeList = new ArrayList<MilitaryUnit>();
		upgradeList.add(new HourseArcher());
		upgradeList.add(new Knight());
		requireTech = new ArrayList<String>();
		requireTech.add("TheWheel");
	...
	public Chariot(int exp, List<String> prom){
		super(exp, new ArrayList<String>());
		this.name="Chariot";
		this.unitType = "Mounted";
		this.baseMovement = 2;
	...
	public MilitaryUnit upgrade(String s) {
		if(s.equals("HourseArcher")) return new HourseArcher(exp,promotions);
		else if (s.equals("Knight")) return new Knight(exp,promotions);
		return null;
	}
}
